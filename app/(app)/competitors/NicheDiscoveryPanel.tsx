...
type Props = {
  ...
  initialNiches?: DiscoveredNiche[];
  initialFilters?: DiscoveryFilters;
};

/**
 * NicheDiscoveryPanel - Powerful niche discovery tool
 *
...
export default function NicheDiscoveryPanel({
  ...
  const [filters, setFilters] = useState<DiscoveryFilters>(
    initialFilters ?? DEFAULT_DISCOVERY_FILTERS
  );
  const [queryText, setQueryText] = useState("");
  const [niches, setNiches] = useState<DiscoveredNiche[]>(initialNiches ?? []);
  ...
  const fetchNiches = useCallback(
    async (currentFilters: DiscoveryFilters, currentQueryText: string, append = false) => {
  ...
  const handleDiscover = useCallback(() => {
    setNextCursor(null);
    fetchNiches(filters, queryText, false);
  }, [filters, queryText, fetchNiches]);

  const handleLoadMore = useCallback(() => {
    fetchNiches(filters, queryText, true);
  }, [filters, queryText, fetchNiches]);
  ...
  const applyAllFilters = useCallback(() => {
    ...
    setNextCursor(null);
    fetchNiches(draftFilters, draftQueryText, false);
  }, [draftFilters, draftQueryText, fetchNiches]);
      ...
      {!isLoading && !error && !hasSearched && (
        <div className={s.discoveryEmpty}>
          ...
          <p className={s.discoveryEmptyDesc}>
            ...
            something interesting, click "Search this niche" to jump into competitor search.
          </p>
        </div>
      )}

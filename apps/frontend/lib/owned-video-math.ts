...
export function computeDerivedMetrics(
  ...
  // Normalized per 1K views
  const subsPer1k = totals.subscribersGained != null ? totals.subscribersGained / viewsPer1k : null;
  const sharesPer1k = totals.shares != null ? totals.shares / viewsPer1k : null;
  const commentsPer1k = totals.comments != null ? totals.comments / viewsPer1k : null;
 ...
 */
export function computeChannelBaseline(videoMetrics: DerivedMetrics[]): ChannelBaseline {
  ...
  return {
    ...
    watchTimePerViewSec: computeMeanStd(
      videoMetrics.map((m) => m.watchTimePerViewSec).filter((v): v is number => v != null)
    ),
    subsPer1k: computeMeanStd(
      videoMetrics.map((m) => m.subsPer1k).filter((v): v is number => v != null)
    ),
    engagementPerView: computeMeanStd(
      videoMetrics.map((m) => m.engagementPerView).filter((v): v is number => v != null)
    ),
    sharesPer1k: computeMeanStd(
      videoMetrics.map((m) => m.sharesPer1k).filter((v): v is number => v != null)
    ),
  };
}
...
export function compareToBaseline(
  ...
  const comparison: BaselineComparison = {
    ...
    avgViewPercentage: computeZScore(avgViewPercentage, baseline.avgViewPercentage),
    watchTimePerViewSec: computeZScore(metrics.watchTimePerViewSec, baseline.watchTimePerViewSec),
    subsPer1k: computeZScore(metrics.subsPer1k, baseline.subsPer1k),
    engagementPerView: computeZScore(metrics.engagementPerView, baseline.engagementPerView),
...
function computeZScore(
  ...
  if (baseline.std === 0) {
    return {
      ...
      percentile: value === baseline.mean ? 50 : value > baseline.mean ? 84 : 16,
      vsBaseline: value > baseline.mean ? "above" : value < baseline.mean ? "below" : "at",
      delta: baseline.mean !== 0 ? ((value - baseline.mean) / baseline.mean) * 100 : null,
    };
  }

  const zScore = (value - baseline.mean) / baseline.std;
  const percentile = normalCDF(zScore) * 100;
  const delta = baseline.mean !== 0 ? ((value - baseline.mean) / baseline.mean) * 100 : null;

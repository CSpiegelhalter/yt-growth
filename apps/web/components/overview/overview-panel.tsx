...
export function OverviewPanel({
  ...
  const fetchOverview = useCallback(async () => {
    ...
    try {
      const result = await apiFetchJson<ChannelOverviewResult>(
        `${apiBasePath}/${channelId}/overview?range=30d`,
      );
      setData(result);
    } catch {
    ...
    const fetchLlmInsights = async () => {
      ...
      try {
        ...
        const result = await apiFetchJson<LlmInsightsResponse>(
          `${apiBasePath}/${channelId}/overview/insights`,
          {
            method: "POST",
  ...
  }, [data, videos, channelId, apiBasePath]);

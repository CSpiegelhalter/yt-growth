...
// Pending task TTL (1 hour - tasks should complete well before this)
const PENDING_TTL_MS = 60 * 60 * 1000;

// Provider identifier for cache entries
...
// Cache mode - can be any string to support flexible cache keys
type CacheMode = string;

// 24 hour TTL for YouTube SERP (shorter than keyword data since rankings change)
...
// Video Ideas cache response type (imported dynamically to avoid circular deps)
...
export async function getCachedResponse<T extends KeywordOverviewResponse | KeywordRelatedResponse | KeywordCombinedResponse | GoogleTrendsResponse | YouTubeSerpResponse>(
  ...
  if (responseData?.pending === true) {
    // Don't return pending entries as cached - let the caller handle polling
    return null;
  }
...
export async function setCachedResponse(
  ...
  const requestHash = generateRequestHash({ mode, phrase, location });
  const now = new Date();
  // YouTube SERP uses shorter TTL since rankings change more frequently
...
/**
 * Store a pending task in the cache.
 * Used when a Standard task is submitted but not yet complete.
 */
...
/**
 * Get a pending task by taskId.
 * Returns null if no pending task found or if task has completed.
 */
export async function getPendingTask(
  ...
  // Find entry with matching taskId in responseJson
  const entry = await prisma.keywordCache.findFirst({
    where: {
      provider: PROVIDER,
      expiresAt: { gt: now },
...
/**
 * Complete a pending task by storing the results.
 * This converts a pending cache entry to a completed one.
 */
...
/**
 * Generate a cache key for video ideas
 * Uses topic + location + preferences to create unique hash
 */
...
/**
 * Get cached video ideas if available and not expired
 */

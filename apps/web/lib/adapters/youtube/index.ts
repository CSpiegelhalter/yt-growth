/**
 * YouTube Adapter — YouTube Data API & Analytics API I/O layer.
 *
 * Implements (or aligns with) the YouTubePort interface from lib/ports/YouTubePort.ts.
 * Currently exports standalone functions; a formal class-based implementation of
 * the YouTubePort interface can be introduced later.
 *
 * TODO: wrap exports in a class that `implements YouTubePort` so features can
 *       depend on the interface instead of concrete functions.
 */
import "server-only";
